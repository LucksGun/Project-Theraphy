...
const InvoiceManagerPage: React.FC = () => {
    ...
    const handlePrintInvoice = useCallback((invoice: Invoice) => {
        ...
        const printContent = `
            ...
            <body>
                <div class="container">
                    <h1 style="text-align: center; margin-bottom: 20px; color: #333;">INVOICE</h1>
    ...
    const handlePrintReceipt = useCallback((invoice: Invoice) => {
        ...
        if (invoice.paymentMethod === 'cash') { paymentDetailsHtml = '<p>Payment Method: Cash</p>'; }
        else if (invoice.paymentMethod === 'bank') { paymentDetailsHtml = `<p>Payment Method: Bank Transfer</p><p>Reference: ${invoice.paymentReference || 'N/A'}</p>`; }
        const printContent = `
            <html> <head> <title>Receipt ${invoice.id}</title> <style> body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 0; padding: 0; font-size: 12px; color: #333; } .container { max-width: 750px; margin: 20px auto; padding: 30px; border: 1px solid #eee; box-shadow: 0 0 10px rgba(0,0,0,0.05); } .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px; padding-bottom: 15px; border-bottom: 1px solid #eee;} .header .logo { font-size: 1.5em; font-weight: bold; color: #555; } .header .company-details p { margin: 2px 0; font-size: 0.9em; text-align: right; color: #555; } .receipt-info { display: flex; justify-content: space-between; margin-bottom: 30px; } .receipt-info .bill-to p { margin: 2px 0; } .receipt-info .receipt-meta p { margin: 2px 0; text-align: right; } .receipt-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; } .receipt-table th, .receipt-table td { border: 1px solid #eee; padding: 8px; text-align: left; } .receipt-table th { background-color: #f8f9fa; font-weight: bold; } .receipt-table .total-row td { font-weight: bold; border-top: 2px solid #aaa; } .receipt-table .text-right { text-align: right; } .payment-details { margin-top: 20px; padding: 15px; border: 1px dashed #ccc; background-color: #f9f9f9; font-size: 0.95em; } .payment-details h3 { margin-top: 0; margin-bottom: 10px; font-size: 1.1em; } .qr-code-section { display: flex; align-items: center; justify-content: space-between; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;} .qr-code-container { text-align: center; } .qr-code-container p { font-size: 0.8em; margin-top: 5px; word-break: break-all; max-width: 150px; } .notes { margin-top: 20px; font-size: 0.85em; color: #777; } @media print { body { margin: 0; padding: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; } .container { border: none; box-shadow: none; margin: 0; max-width: 100%; padding: 10px; } .no-print { display: none; } } </style> </head>
            <body>
                <div class="container">
                    <h1 style="text-align: center; margin-bottom: 20px; color: #198754;">PAYMENT RECEIPT</h1>
                    <div class="header"> <div class="logo">Project Theraphy</div> <div class="company-details"> <p>01 Sanambin Road</p> <p>Nai Mueng, Phitsanulok 65000</p> <p>088-555-1946</p> <p>thammalucka67@nu.ac.th</p> </div> </div>
                    <div class="receipt-info"> <div class="bill-to"> <strong>Received From:</strong><br> ${invoice.customerName} </div> <div class="receipt-meta"> <p><strong>Receipt #:</strong> ${invoice.id}</p> <p><strong>Payment Date:</strong> ${new Date().toLocaleDateString()}</p> <p><strong>Status:</strong> <strong style="color: #198754;">${invoice.status}</strong></p> </div> </div>
                    <table class="receipt-table"> <thead> <tr> <th>Description</th> <th class="text-right">Amount Paid</th> </tr> </thead> <tbody> ${itemRowsHtml} <tr class="total-row"> <td class="text-right"><strong>Total Paid:</strong></td> <td class="text-right"><strong>$${totalAmount.toFixed(2)}</strong></td> </tr> </tbody> </table>
                    <div class="payment-details"> <h3>Payment Details</h3> ${paymentDetailsHtml} </div>
                    <div class="qr-code-section"> <div class="notes"> Thank you for your payment! </div> <div class="qr-code-container"> <div id="qr-code-target"></div> <p>${invoice.id}</p> </div> </div>
                </div>
                                    ...
                                    return (
                                        <tr key={invoice.id}>
                                            ...
                                            <td>
                                                ...
                                                <button onClick={() => handlePrintInvoice(invoice)} className="table-button print-invoice" disabled={isSubmitting}>Invoice</button>
                                                <button onClick={() => handlePrintReceipt(invoice)} className="table-button print-receipt" disabled={isSubmitting || !isPaid} title={isPaid ? "Print Receipt" : "Mark as Paid to print receipt"}>Receipt</button>
                                            </td>
                                        </tr>
                                    );
                                })}
