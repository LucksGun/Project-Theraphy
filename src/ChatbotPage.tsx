// src/ChatbotPage.tsx - Includes Syntax Highlighting, Loading Indicator, GA Import Fix, Full Analysis Form JSX
import React, { useState, useRef, useEffect, useCallback, memo } from 'react'; // Added memo
import ReactGA from 'react-ga4'; // Import ReactGA HERE because it's used in handleAnalysisSubmit
...
// For true theme switching, you'd conditionally import/use a dark style (e.g., okaidia, vscDarkPlus).
...
function ChatbotPage({ messages, setMessages, selectedModel, sttLang, selectedPersona, accessKey }: ChatbotPageProps) {
    ...
    const removeSelectedImage = () => { setSelectedImage(null); if (imagePreviewUrl) URL.revokeObjectURL(imagePreviewUrl); setImagePreviewUrl(null); if (fileInputRef.current) fileInputRef.current.value = ""; };
    ...
    const CodeRenderer = memo(({ node, inline, className, children, ...props }: any) => {
        ...
        return !inline && match ? (
            <SyntaxHighlighter
                style={prism} // Using prism style (light). Consider conditional style for dark mode later.
                language={match[1]}
                PreTag="div" // Use div wrapper, custom styling applied via CSS `.message pre`
                {...props}
            >
                {codeText}
            </SyntaxHighlighter>
    ...
    CodeRenderer.displayName = 'CodeRenderer'; // Add display name for React DevTools
    ...
    return (
        <div className="chatbot-container">
            <div className="chatbot-messages">
                                ...
                                {message.sender === 'bot' ? (
                                    <>
                                        {isErrorMessage ? (
                                            ...
                                            // --- Use ReactMarkdown with custom CodeRenderer ---
                                            <ReactMarkdown
                                                remarkPlugins={[remarkGfm]}
                                                children={mainText}
                                                components={{ code: CodeRenderer }} // Override code rendering
                                            />
                                        ) : null}
                                        {message.imageUrl && ( <img src={message.imageUrl} alt="Bot response" className="bot-image" onClick={() => window.open(message.imageUrl, '_blank')} onError={(e) => { console.warn(`Failed image load: ${message.imageUrl}`); const imgElement = e.target as HTMLImageElement; imgElement.style.display = 'none'; const errorText = document.createElement('span'); errorText.textContent = '[Image failed to load]'; errorText.style.fontSize = '0.8em'; errorText.style.color = 'grey'; imgElement.parentNode?.insertBefore(errorText, imgElement.nextSibling); }}/> )}
                                    ...
                                    // User message: Use standard paragraph for potential wrapping
             ...
             {/* Analysis Form Overlay - FULL JSX INCLUDED */}
                        ...
                        <form onSubmit={handleAnalysisSubmit}>
                            {/* Fields 1-5 */}
                             <div className="settings-option" style={{marginBottom:'15px'}}>
                                 <label htmlFor="analysis-field1">1. Concerns about university?</label>
                                 <input type="text" id="analysis-field1" className="settings-input" value={field1} onChange={(e)=>setField1(e.target.value)} placeholder="e.g., workload, social life, cost" disabled={isAnalyzing} required />
                             </div>
                             <div className="settings-option" style={{marginBottom:'15px'}}>
                                 <label htmlFor="analysis-field2">2. Who do you enjoy spending time with?</label>
                                 <input type="text" id="analysis-field2" className="settings-input" value={field2} onChange={(e)=>setField2(e.target.value)} placeholder="e.g., close friends, family, alone" disabled={isAnalyzing} required />
                             </div>
                             <div className="settings-option" style={{marginBottom:'15px'}}>
                                 <label htmlFor="analysis-field3">3. Describe your personality?</label>
                                 <input type="text" id="analysis-field3" className="settings-input" value={field3} onChange={(e)=>setField3(e.target.value)} placeholder="e.g., introverted, creative, analytical" disabled={isAnalyzing} required />
                             </div>
                             <div className="settings-option" style={{marginBottom:'15px'}}>
                                 <label htmlFor="analysis-field4">4. What learning methods do you dislike?</label>
                                 <input type="text" id="analysis-field4" className="settings-input" value={field4} onChange={(e)=>setField4(e.target.value)} placeholder="e.g., memorization, group projects, exams" disabled={isAnalyzing} required />
                             </div>
                             <div className="settings-option" style={{marginBottom:'15px'}}>
                                 <label htmlFor="analysis-field5">5. Estimated GPA or academic level?</label>
                                 <input type="text" id="analysis-field5" className="settings-input" value={field5} onChange={(e)=>setField5(e.target.value)} placeholder="e.g., 3.5, Good, Average" disabled={isAnalyzing} required />
                             </div>
