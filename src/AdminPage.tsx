...
export interface UserWithHistory { id: string; username: string; email: string | null; last_updated: string; }
...
function AdminPage() {
    ...
    const fetchAdminData = async (staffKey: string | null) => {
        ...
        try {
            ...
            const listUsersWithHistoryBody: ApiRequestBody = { action: 'adminListUsersWithHistory', staffKey: staffKey };
            ...
            const promptsData = await promptsRes.json().catch(() => ({ error: 'Invalid JSON prompts' }));
            if (!promptsRes.ok || !promptsData.success) throw new Error(promptsData?.error || 'Failed fetch prompts.');
            ...
            setBasePrompt(fetchedBase);
            setInitialBasePrompt(fetchedBase);
            setPersonaPrompts(fetchedPersonas);
            setInitialPersonaPrompts(fetchedPersonas);
            
            const usersWithHistoryData = await usersWithHistoryRes.json().catch(() => ({ error: 'Invalid JSON for users with history' }));
            ...
            setAdminRestrictedPersonasList([]);
            setAdminFeedbackList([]);
            setUsersWithHistory([]);
            setBasePrompt(DEFAULT_BASE_SYSTEM_INSTRUCTION);
            setPersonaPrompts(DEFAULT_PERSONA_INSTRUCTIONS);
            setInitialBasePrompt(DEFAULT_BASE_SYSTEM_INSTRUCTION);
            setInitialPersonaPrompts(DEFAULT_PERSONA_INSTRUCTIONS);
        } finally {
        ...
        <div className="admin-page-container">
            ...
            <div className="staff-admin-section">
                ...
                <div className="admin-data-section">
                    <p className="restriction-description">View or clear chat histories for registered users. Histories are ordered by most recently updated.</p>
                                        ...
                                        <tr key={user.id}>
                                            ...
                                            <td>
                                                <div className="action-buttons-cell">
                                                    <button 
                                                        onClick={() => handleFetchHistory(user.id)} 
