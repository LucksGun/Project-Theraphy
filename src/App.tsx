    ...
    const fetchAdminData = async () => {
        ...
        try {
            ...
            const [keysRes, restrictRes] = await Promise.all([
                fetch(WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'adminListKeys', staffKey: enteredStaffKey }) }), // Pass staff key
                fetch(WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'adminGetRestrictions', staffKey: enteredStaffKey }) }) // Pass staff key
            ]);
    ...
    const handleToggleUserKeyStatus = async (key: string, currentStatus: 'active' | 'inactive') => {
        ...
        try {
            const res = await fetch(WORKER_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'adminUpdateKeyStatus', staffKey: enteredStaffKey, key: key, newStatus: newStatus }) // Pass staff key
            });
    ...
    const handleToggleModelRestriction = async (modelValue: GeminiModel) => {
        ...
        try {
            const res = await fetch(WORKER_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                // Send the *entire* new list of restricted models and staff key
                body: JSON.stringify({ action: 'adminSetRestrictedModels', staffKey: enteredStaffKey, models: newList })
            });
                        ...
                        {!isStaffAuthenticated ? (
                            ...
                            <div className="staff-admin-section">
                                ...
                                <div className="admin-data-section">
                                    {isAdminLoading && !adminUserKeysList.length && <p>Loading user keys...</p>} {/* Show loading only if list empty */}
                                ...
                                <h4>Manage Restricted Models</h4>
                                 <div className="admin-data-section">
                                     {isAdminLoading && !adminRestrictedModelsList.length && <p>Loading model restrictions...</p>} {/* Show loading only if list empty */}
                                     ...
                                     {!isAdminLoading && !adminError && ALL_AVAILABLE_MODELS_FRONTEND.length > 0 && (
                                         <div className="restricted-models-list">
                                             <p style={{fontSize: '0.9em', color: '#666', marginBottom: '10px', padding: '0 12px'}}>Toggle which AI models require a valid user access key.</p>
