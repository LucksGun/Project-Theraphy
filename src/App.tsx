// src/App.tsx - FINAL COMPLETE Version v11 (No Placeholders)
...
function App() {
    ...
    const toggleSettings=()=>{ setIsSettingsOpen(p=>!p); setIsStaffLoginModalVisible(false); setIsFeedbackModalVisible(false); setIsClearCupGameVisible(false); setIsModelBuilderVisible(false); setIsPersonaPlinkoVisible(false); };
    ...
    const toggleStaffLoginModal = () => { setIsStaffLoginModalVisible(prev => !prev); if (isStaffLoginModalVisible) { setEnteredStaffKey(''); setStaffLoginError(null); } setIsSettingsOpen(false); setIsFeedbackModalVisible(false); setIsClearCupGameVisible(false); setIsModelBuilderVisible(false); setIsPersonaPlinkoVisible(false);};
        ...
        <div className="App">
            ...
            {isSettingsOpen && ( <div className="settings-menu" role="dialog" aria-labelledby="settings-title"><h3 id="settings-title">Settings</h3><div className="settings-grid">
                ...
                <div className="settings-column">
                    ...
                    {/* Persona Setting - Uses Plinko Game */}
                    <div className="settings-option"><label>Persona:</label><p className="current-persona-display">{AVAILABLE_PERSONAS.find(p=>p.value === selectedPersona)?.emoji} {AVAILABLE_PERSONAS.find(p=>p.value === selectedPersona)?.label || selectedPersona}</p><button onClick={() => setIsPersonaPlinkoVisible(true)} className="settings-action-button persona-change-button" disabled={!canChangePersona} title={ !canChangePersona ? "Requires a valid Access Key..." : "Change Persona (Opens Game)"}>Change Persona</button>{!canChangePersona && (<p className="settings-helper-text">Enter key to change.</p>)}</div>
                    {/* AI Model Setting - Uses Builder Game */}
                ...
                <div className="settings-column">
                    ...
                    {/* Appearance Setting - Simple Toggle Button */}
                    ...
                    <div className="settings-option"> <label>Chat Actions:</label> <div style={{display: 'flex', flexDirection: 'column', gap: '8px'}}><button onClick={handleExportChat} className="settings-action-button export-chat-settings-button">💾 Export Chat</button>
                    {/* Clear Chat Button - Uses Cup Game */}
                    <button onClick={() => setIsClearCupGameVisible(true)} className="settings-action-button clear-chat-settings-button">🗑️ Clear Chat History</button>
                    </div> </div>
